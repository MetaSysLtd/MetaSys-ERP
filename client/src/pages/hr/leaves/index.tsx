...
import { Loader2, Calendar, FileCheck, Ban } from 'lucide-react';
...
function NewLeaveRequestForm() {
  ...
  const createLeaveRequestMutation = useMutation({
    mutationFn: async (formData: any) => {
      const response = await apiRequest('POST', '/api/hr/leaves/requests', formData);
      if (!response.ok) {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/hr/leaves/requests'] });
      queryClient.invalidateQueries({ queryKey: ['/api/hr/leaves/balances'] });
    },
    onError: (error: Error) => {
      setIsSubmitting(false);
      toast({
...
function LeaveRequestsTable() {
  ...
  const { data: leaveRequests, isLoading, error } = useQuery<LeaveRequest[]>({
    queryKey: ['/api/hr/leaves/requests'],
    queryFn: async () => {
      const response = await fetch('/api/hr/leaves/requests');
      if (!response.ok) {
  ...
  const approveRequestMutation = useMutation({
    mutationFn: async (requestId: number) => {
      const response = await apiRequest('PATCH', `/api/hr/leaves/requests/${requestId}`, { status: 'Approved' });
      if (!response.ok) {
        const errorData = await response.json();
  ...
  const rejectRequestMutation = useMutation({
    mutationFn: async ({ requestId, reason }: { requestId: number, reason: string }) => {
      const response = await apiRequest('PATCH', `/api/hr/leaves/requests/${requestId}`, { 
        status: 'Rejected',
  ...
  const cancelRequestMutation = useMutation({
    mutationFn: async (requestId: number) => {
      const response = await apiRequest('POST', `/api/hr/leaves/requests/${requestId}/cancel`, {});
      if (!response.ok) {
        const errorData = await response.json();
...
function LeaveBalanceCard() {
  const { data: leaveBalance, isLoading, error } = useQuery<LeaveBalance>({
    queryKey: ['/api/hr/leaves/balances'],
    queryFn: async () => {
      const response = await fetch('/api/hr/leaves/balances');
      if (!response.ok) {
...
function LeavePoliciesTable() {
  const { data: policies, isLoading, error } = useQuery<LeavePolicy[]>({
    queryKey: ['/api/hr/leaves/policies'],
    queryFn: async () => {
      const response = await fetch('/api/hr/leaves/policies');
      if (!response.ok) {
  ...
  if (!policies || policies.length === 0) {
    return <p>No leave policies found.</p>;
  }
